...
export class ExperimentsFetcherService
  ...
  protected fetchAsset(search: SearchOptions, sort: Sort, page: PageEvent): Observable<ListWrapper<ExperimentSummary>> {
    ...
    return this.experimentService.searchExperiments(query, search.species, search.author, search.executedFrom, search.executedTo, search.dataCategory, search.showPublic, sort, page);
  }
  ...
  protected sameInput(def1: SearchOptions, def2: SearchOptions): boolean {
    return ((def1.showPublic === def2.showPublic)
            && (def1.query === def2.query)
            && (def1.species === def2.species)
            && (def1.author === def2.author)
            && (def1.executedFrom === def2.executedFrom)
            && (def1.executedTo === def2.executedTo)
            && (def1.dataCategory === def2.dataCategory));
  }
